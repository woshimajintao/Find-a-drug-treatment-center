...
function initMap(lat, lng) {
    ...
    addMarker(location, "My Location", {
        url: "drug-addiction.png",
        scaledSize: new google.maps.Size(32, 32) // Adjust the size as needed
    }, "My Location"); // Use the custom icon for the current location
}
            ...
            if (data.results && data.results.length > 0) {
                ...
                centers.forEach((center) => {
                    ...
                    if (lat && lng) {
                        ...
                        const photoUrl = center.photos && center.photos.length > 0 ? center.photos[0].photo_reference : null;
                        const content = `<div><strong>${center.name}</strong><br>${center.formatted_address}<br>Rating: ${center.rating || 'N/A'}<br>${photoUrl ? `<img src="https://maps.googleapis.com/maps/api/place/photo?maxwidth=400&photoreference=${photoUrl}&key=${GOOGLE_API_KEY}" alt="${center.name}">` : ''}</div>`;
                        addMarker({ lat: centerLat, lng: centerLng }, center.name, {
                            url: "Drug rehabilitation center.png",
                            scaledSize: new google.maps.Size(32, 32) // Adjust the size as needed
                        }, content); // Add marker for each center with info window
                    }
                });
            } else {
                console.log("No rehabilitation centers found in the specified area.");
            }
        })
        .catch((error) => {
...
function displayHotlineInfo(country) {
    ...
    if (hotlineInfo) {
        resultsDiv.innerHTML = `<h1>Contact Information for Immediate Support:</h1>
                                <p>If you are in ${country} and need help, here are the ways to get in touch:</p>
                                <p><img src="helpline.png" style="width: 20px; height: 20px; vertical-align: middle;"> Hotline: ${hotlineInfo.hotline}</p>
                                <p><img src="mail.png" style="width: 20px; height: 20px; vertical-align: middle;"> Email: ${hotlineInfo.email}</p>`;
    } else {
